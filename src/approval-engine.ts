...
export interface ApprovalConfig {
  autoApproveTokenBudget: number;      // Token消耗预算比例阈值
  autoApproveSubtaskImpact: number;    // 影响子任务数量阈值
  autoApproveFixTime: number;          // 修复时间阈值（秒）
  maxRetries: number;                  // 最大回退次数
}
...
export interface ApprovalContext {
  ...
  affectedSubtasks?: number;           // 受影响的子任务数量
  fixTimeSeconds?: number;             // 修复所需时间（秒）
  tokenBudgetRatio?: number;           // Token消耗与预算比例
}
...
export class ApprovalEngine {
   ...
   * 检查是否需要人工审批
   */
  private needsHumanReview(context: ApprovalContext): boolean {
    // 1. 超过最大回退次数
   ...
   * 检查自动审批条件
   */
  private checkAutoApprovalConditions(context: ApprovalContext): { approved: boolean; reason: string } {
   ...
   * 触发人工审批流程
   */
  manualApprovalRequest(taskId: string, context: ApprovalContext): { requested: boolean; message: string } {
   ...
   * 生成人工审批消息
   */
  private generateApprovalMessage(taskId: string, context: ApprovalContext): string {
    return `
【人工审批请求】
任务ID: ${taskId}
